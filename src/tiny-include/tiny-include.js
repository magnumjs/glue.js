/*
Name: glue-importer v0.1.4
Description: similar to html-imports implementation
Author: Michael Glazer
License: MIT
requires: glue.request (tiny-ajax)
Homepage: https://github.com/magnumjs/glue.js
(c) 2016
*/

;
(function(glue, document) {

  'use strict';

  glue.importer = function(url, eleSelector) {
    ...
    function getId(id) {
      if (typeof glue.importer.ids[id] !=='undefined') {
        return id + (++glue.importer.ids[id]);
      } else {
        return id + (glue.importer.ids[id] = 0);
      }
    }
      ...
      for (var i = 0; i < scripts.length; i++) {
        ...
        if (url != "") {
          ...
          if (checkIfIncluded(url, node)) {
            promises.push(glue.importer.namedPromises[url].promise);
            continue;
          }

          var deferred = glue.deferred();
          if (!names[url]) {
            glue.importer.namedPromises[url] = names[url] = deferred;
          } else continue;
